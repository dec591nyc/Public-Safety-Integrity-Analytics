...
const combineOtherSegments = (segments, total) => {
  ...
  for (const seg of segments) {
    if (seg.metric === '__other__' || seg.label === '其他案件' || seg.metric === '其他' || seg.label === '其他') {
        ...
        <div key={index} className="change-card">
          <div className="change-head">
            <div>
              ...
              <small>{row.period_label} · 全部官方案件差額拆解</small>
            </div>
          ...
          <div className="reconcile-line">
            ...
            <span>案件差額加總 <span>{formatSignedCount(row.metric_delta_sum)}</span></span>
    ...
    <div className="panel" data-view-panel="year">
      ...
      {activeTab === 'kpi' && (
        <>
          ...
          <div className="annual-peak-section" style={{ marginTop: '14px' }}>
            <span className="subhead">每年高峰月與主要案件</span>
              ...
              {(annualComparison.peak_months || [])
                ...
                .map((row, index) => {
                  ...
                  for (const seg of combined) {
                    if (seg.metric === '__other__' || seg.label === '其他案件') {
                      otherCount += Number(seg.count || 0);
                    } else {
                  ...
                  return (
                    <div key={index} className="peak-row">
                      ...
                      <div className="peak-check">
                        ...
                        <span>其餘案件 {fmt.format(otherCount)} 件</span>
      ...
      {activeTab === 'same' && (
        <div className="annual-change-section">
          <span className="subhead">同期間變化拆解：哪些案件推動 KPI 年增率 (YoY)？</span>
