...
export default function DrilldownDetail({
  ...
  const drillScopeNote = activeTopic.is_total_scope
    ? `目前「${activeTopic.label}」為全部官方案件加總；「${selectedMetricRecord?.label || '未選案件'}」是其中一個單一案件。`
    : activeTopic.is_residual_scope
      ? `目前「${activeTopic.label}」為前六個分析主題之外的官方案件集合；「${selectedMetricRecord?.label || '未選案件'}」是其中一個單一案件。`
      : `目前「${activeTopic.label}」為主題包合計；「${selectedMetricRecord?.label || '未選案件'}」是單一案件。兩者資料範圍不同，例如性犯罪與家庭會把妨害性自主罪、妨害風化、妨害家庭及婚姻、遺棄合併。`;
  ...
  return (
    <>
      <div className="metric-picker">
        <span className="subhead">案件選擇</span>
      ...
      <div className="drill-path" aria-label="目前細究路徑" style={{ margin: '10px 0' }}>
        ...
        <b>›</b>
        <span>{selectedMetricRecord?.label || '未選案件'}</span>
      </div>
      ...
      <div className="mini-stat-grid drill-stats">
        ...
        <div className="mini-stat">
          <span>{scopeLabel}選定案件</span>
          <strong>{fmt.format(countyMetricCount)} 件</strong>
          <small>{selectedMetricRecord ? selectedMetricRecord.label : '未選案件'}</small>
        </div>
        <div className="mini-stat">
          <span>全國選定案件</span>
      ...
      <div className="drill-columns" style={{ marginTop: '12px' }}>
        <div>
          <span className="subhead">
            {selectedRegionRecord?.isNational ? '全國案件構成' : '該縣市案件構成'}
          </span>
        ...
        <div>
          <span className="subhead">此案件前六縣市與其他縣市</span>
      ...
      <p className="method-note">目前為案件數量呈現，尚未除以人口、戶數或日數；縣市比較應避免直接等同於風險排名。</p>
    </>
  );
}
