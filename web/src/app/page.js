...
export default function DashboardPage() {
  ...
  const renderTopicAiAnalysis = (topic) => {
    ...
    const scopeNote = topic.is_total_scope
      ? '此分頁採全部官方案件加總，資料範圍可與全國總案量曲線核對。'
      : topic.is_residual_scope
        ? '此分頁收納未放入前六個分析主題的官方案件，用來補足主題分類外的案件。'
    ...
    const mainCategoryNote = mainSegment ? `最新月之主要案件為「${mainSegment.label}」（${fmt.format(mainSegment.count)} 件）。` : '';
  ...
  const renderAiInsightPanel = () => {
    ...
    return (
      <div className="ai-insight">
        ...
        <span className="subhead" style={{ marginTop: '14px', display: 'block' }}>主要治安觀察案件</span>
            ...
            {activeView === 'local' && (
              <section className="dashboard-grid local-workspace">
                <article className="panel drill-panel">
                  <div className="panel-head">
                    <div>
                      <p className="section-kicker">縣市狀況路徑</p>
                      <h3>縣市與案件狀況</h3>
                      <p>選擇主題、縣市與案件後，查看該縣市案件類型的比例與排名。</p>
                    </div>
                    <span className="chart-type">主題 / 縣市 / 案件</span>
                  </div>
              ...
              <section className="dashboard-grid context-grid">
                <article className="panel">
                  ...
                  <p className="method-note inline" style={{ marginTop: '12px' }}>
                    此曲線每一點代表該月份全國總案量；不同月份不可相加後拿來對照目前選取月份的案件構成。
                  </p>
                </article>
