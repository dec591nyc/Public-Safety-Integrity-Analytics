...
async function loadMonths() {
  ...
  state.monthsList = data.items?.length ? data.items : [{ source_month: '202604', count: 0 }];
  populateMonthDropdown();
}

function populateMonthDropdown() {
  const select = el('month');
  if (!select) return;

  const currentSelection = select.value || (state.activeDimension === 'year' ? '2025_annual' : '202604');
  
  // Filter based on activeDimension
  let items = [];
  if (state.activeDimension === 'year') {
    items = state.monthsList.filter(item => item.source_month.endsWith('_annual'));
  } else {
    items = state.monthsList.filter(item => !item.source_month.endsWith('_annual'));
  }
  ...
  // Update selection
  const hasSelectedValue = items.some(item => item.source_month === currentSelection);
  if (hasSelectedValue) {
    select.value = currentSelection;
  } else if (items.length > 0) {
    select.value = items[0].source_month;
  }
}

function updateDimensionSelector() {
  const dimMonthBtn = el('dim-month');
  const dimYearBtn = el('dim-year');
  const selectLabel = el('select-label-text');
  
  if (state.activeDimension === 'year') {
    if (dimMonthBtn) dimMonthBtn.classList.remove('is-active');
    if (dimYearBtn) dimYearBtn.classList.add('is-active');
    if (selectLabel) selectLabel.textContent = '選擇年度';
  } else {
    if (dimMonthBtn) dimMonthBtn.classList.add('is-active');
    if (dimYearBtn) dimYearBtn.classList.remove('is-active');
    if (selectLabel) selectLabel.textContent = '選擇月份';
  }
}
...
function bindEvents() {
  ...
  const setDimension = (dim) => {
    if (state.activeDimension === dim) return;
  ...
  if (dimMonthBtn) {
    dimMonthBtn.addEventListener('click', () => setDimension('month'));
  }
  if (dimYearBtn) {
    dimYearBtn.addEventListener('click', () => setDimension('year'));
  }
}
